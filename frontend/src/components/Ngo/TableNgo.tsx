...
import {
  ...
  setRefetch,
  deleteItemsByIds,
} from '../../stores/ngo/ngoSlice';
import { useAppDispatch, useAppSelector } from '../../stores/hooks';
import { useRouter } from 'next/router';
import { Field, Form, Formik } from 'formik';
import { DataGrid, GridColDef } from '@mui/x-data-grid';
import { loadColumns } from './configureNgoCols';
...
const TableSampleNgo = ({ filterItems, setFilterItems, filters, showGrid }) => {
  ...
  const {
    ...
    notify: ngoNotify,
    refetch,
  } = useAppSelector((state) => state.ngo);
  ...
  useEffect(() => {
    if (ngoNotify.showNotification) {
      notify(ngoNotify.typeNotification, ngoNotify.textNotification);
    }
  }, [ngoNotify.showNotification]);
  ...
  const handleViewAction = (id: string) => {
    router.push(`/ngo/ngo-view/?id=${id}`);
  };
