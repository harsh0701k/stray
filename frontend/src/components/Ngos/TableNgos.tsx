...
import {
  ...
  setRefetch,
  deleteItemsByIds,
} from '../../stores/ngos/ngosSlice';
import { useAppDispatch, useAppSelector } from '../../stores/hooks';
import { useRouter } from 'next/router';
import { Field, Form, Formik } from 'formik';
import { DataGrid, GridColDef } from '@mui/x-data-grid';
import { loadColumns } from './configureNgosCols';
import _ from 'lodash';
import dataFormatter from '../../helpers/dataFormatter';
import { dataGridStyles } from '../../styles';

import ListNgos from './ListNgos';

const perPage = 10;

const TableSampleNgos = ({
  filterItems,
  setFilterItems,
  filters,
  showGrid,
  ...
  const {
    ...
    notify: ngosNotify,
    refetch,
  } = useAppSelector((state) => state.ngos);
  ...
  useEffect(() => {
    if (ngosNotify.showNotification) {
      notify(ngosNotify.typeNotification, ngosNotify.textNotification);
    }
  }, [ngosNotify.showNotification]);
  ...
  const handleViewAction = (id: string) => {
    router.push(`/ngos/ngos-view/?id=${id}`);
  };
  ...
  return (
    <>
      ...
      {ngos && Array.isArray(ngos) && !showGrid && (
        <ListNgos
          ngos={ngos}
          loading={loading}
...
export default TableSampleNgos;
