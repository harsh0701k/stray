...
export default {
  ...
  animalsManyListFormatter(val) {
    if (!val || !val.length) return [];
    return val.map((item) => item.name);
  },
  animalsOneListFormatter(val) {
    if (!val) return '';
    return val.name;
  },
  animalsManyListFormatterEdit(val) {
    if (!val || !val.length) return [];
    return val.map((item) => {
      return { id: item.id, label: item.name };
    });
  },
  animalsOneListFormatterEdit(val) {
    if (!val) return '';
  ...
  ngoManyListFormatter(val) {
    if (!val || !val.length) return [];
    return val.map((item) => item.name);
  },
  ngoOneListFormatter(val) {
    if (!val) return '';
    return val.name;
  },
  ngoManyListFormatterEdit(val) {
    if (!val || !val.length) return [];
    return val.map((item) => {
      return { id: item.id, label: item.name };
    });
  },
  ngoOneListFormatterEdit(val) {
    if (!val) return '';
    return { label: val.name, id: val.id };
  },
};
