...
const EditAnimals = () => {
  ...
  const { animalsId } = router.query;

  useEffect(() => {
    dispatch(fetch({ id: animalsId }));
  }, [animalsId]);
  ...
  const handleSubmit = async (data) => {
    await dispatch(update({ id: animalsId, data }));
...
EditAnimals.getLayout = function getLayout(page: ReactElement) {
  return (
